...
class DDPMSWGLearner(Agent):
    ...
    def create(
        ...
        preprocess_time_cls = partial(
            FourierFeatures, output_size=cfg.diffusion.time_emb, learnable=True
        )
        ...
        actor_params = actor_def.init(actor_key, observations, actions_weight, time)[
            "params"
        ]
        ...
        value = TrainState.create(
            apply_fn=value_def.apply, params=value_params, tx=value_optimizer
        )
        ...
        if cfg.diffusion.schedule in schedulers:
            betas = jnp.array(
                schedulers[cfg.diffusion.schedule](cfg.diffusion.n_timesteps)
            )
        else:
        ...
        alphas = 1 - betas
        alpha_hat = jnp.array(
            [jnp.prod(alphas[: i + 1]) for i in range(cfg.diffusion.n_timesteps)]
        )
    ...
    def update_v(agent, batch: DatasetDict) -> tuple[Agent, dict[str, float]]:
        ...
        def value_loss_fn(value_params) -> tuple[jnp.ndarray, dict[str, float]]:
            ...
            value_loss = expectile_loss(
                q - v, agent.critic_hyperparam
            ).mean()  # agent.value_objective_fn(q - v).mean()
    ...
    def update_actor(agent, batch: DatasetDict) -> tuple[Agent, dict[str, float]]:
        ...
        time = jax.random.randint(
            key, (batch["action_weight"].shape[0],), 0, agent.n_timesteps
        )

        key, rng = jax.random.split(rng, 2)
        noise_sample = jax.random.normal(
            key, (batch["action_weight"].shape[0], agent.out_dim)
        )  # B,A+1

        alpha_hats = agent.alpha_hats[time]
        time = jnp.expand_dims(time, axis=1)
        alpha_1 = jnp.expand_dims(
            jnp.sqrt(alpha_hats), axis=1
        )  # TODO possible speed up here precalculating alpha 1 and 2
        alpha_2 = jnp.expand_dims(jnp.sqrt(1 - alpha_hats), axis=1)

        noisy_actions_weights = (
            alpha_1 * batch["action_weight"] + alpha_2 * noise_sample
        )
    ...
    def eval_actions(
        self, observations: jnp.ndarray, rng: PRNGKey, sample_params: dict
    ) -> tuple[jnp.ndarray, PRNGKey]:

        assert len(observations.shape) == 1
        observations = jax.device_put(observations)
        observations = jnp.expand_dims(observations, axis=0).repeat(
            sample_params["batch_size"], axis=0
        )

        score_params = self.target_score_model.params
        actions_weights, rng = ddpm_sampler_swg(
