...
def ddpm_sampler(
    ...
    input_tuple, () = jax.lax.scan(
        fn,
        (jax.random.normal(key, (batch_size, act_dim)), rng) * temperature,
        jnp.arange(T - 1, -1, -1),
    )
...
def ddpm_sampler_swg(
    ...
    def sampling_fn(input_tuple, time):
        ...
        eps_pred = jax.lax.cond(
            guidance_scale != 0, swg_guidance, no_guidance, current_x
        )
        ...
        if clip_denoised:
            ...
            current_x = current_x.at[:, -1].set(
                jnp.clip(current_x[:, -1], min_weight_clip, max_weight_clip)
            )
    ...
    input_tuple, () = jax.lax.scan(
        sampling_fn,
        (jax.random.normal(key, (batch_size, act_dim)) * temperature, rng),
        jnp.arange(T - 1, -1, -1),
    )
